...
async function main() {
    ...
    let newRepoString = JSON.stringify(newRepo);
    ...
    // console.log(pushlog)
    await submitTxnContract(contract,newRepoString)
...
async function queryContract(contract){
    try{
        // Query the chaincode
        let response = await contract.evaluateTransaction('getRepo', 'Hassan', 'testRepo')
...
async function submitTxnContract(contract, pushlog){
    try{
        // Submit the transaction
        let response = await contract.submitTransaction('addNewRepo', pushlog)
