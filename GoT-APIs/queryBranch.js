...
async function queryContract(contract){
    try{
        ...
        int maxTime = 0
        int maxInd = -1
        for(var i = 0; i < responseObj['logs'].length; i++) {
          if (responseObj['logs'][i]['CommitterTimestamp'] >= maxTime){
            maxInd = i
            maxTime = responseObj['logs'][i]['CommitterTimestamp']
          }
        }
        console.log(`LastCommitHash=${responseObj['logs'][maxInd]['hash']}`)
